...
def get_db_connection():
    ...
    conn.row_factory = sqlite3.Row   # dictionary=True replacement
    return conn
...
def admin_login():
    ...
    if admin['status'] != 'approved':
        cursor.close()
        conn.close()
        flash("Your account is pending approval.", "danger")
        return redirect('/admin-login')
    ...
    role = admin['role'].strip().lower()

    if role == 'superadmin':
       
       session['superadmin'] = True

    # ✅ Update login count (make sure column exists)
    cursor.execute("""
        UPDATE admin 
...
def verify_payment():
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, razorpay_order_id, razorpay_payment_id, total_amount, 'paid', session.get('selected_address') ))
...
def order_success(order_db_id):
    ...
    # ✅ Fetch Address using address_id from order
    cursor.execute("""
...
def select_address():
    ...
    cursor.execute("SELECT * FROM addresses WHERE user_id=?", (session['user_id'],))
    addresses = cursor.fetchall()

    cursor.close()
    conn.close()

    return render_template("user/select_address.html",
                           addresses=addresses)
...
def add_address():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            ...
            (user_id, full_name, phone, house_no, street, city, state, pincode)
            VALUES (?,?,?,?,?,?,?,?)
        """, (session['user_id'], full_name, phone, house_no,
...
@app.route('/confirm-address/<int:address_id>')
def confirm_address(address_id):
...
def superadmin_dashboard():
    ...
    # 1️⃣ Total Revenue
    cursor.execute("SELECT SUM(amount) AS total_revenue FROM orders WHERE payment_status='paid'")
    revenue = cursor.fetchone()['total_revenue'] or 0

    # 2️⃣ Total Admins
    cursor.execute("SELECT COUNT(*) AS total_admins FROM admin WHERE role='admin'")
    total_admins = cursor.fetchone()['total_admins']

    # 3️⃣ Admin List
    cursor.execute("""
        SELECT admin_id, name, email, status, login_count 
        FROM admin
        WHERE role='admin'
    """)
...
@app.route('/superadmin/approve/<int:admin_id>')
...
@app.route('/superadmin/reject/<int:admin_id>')
...
def superadmin_products():
    ...
    # Fetch all products + admin who added them
    cursor.execute("""
        SELECT 
            p.product_id,
